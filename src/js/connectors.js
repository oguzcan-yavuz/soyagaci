...
function convertDate(date_string) {
    // Tarihi gun/ay/yil formatindan ay/gun/yil formatina cevirir.
	if(date_string === "") {
		return new Date();
	} else {
        var values = date_string.split("/");
        var temp = values[0];
        values[0] = values[1];
        values[1] = temp;
        return values.join("/");
...
$(function(){
    $('p').click(function(){
		...
		// yas hesaplari
		// TODO: "Ölüm -" seklinde sonuc gosterenlerin dogum tarihi ile olum tarihini esitle, yaslari
		// 0 olsun. bu sayede ortalama hesabi yaparken ayiklamak kolay olur ve kullaniciya
		// "hesaplanamadi" seklinde bir mesaj gosterebilmek icin yasi 0 olanlari kontrol ederiz.

        var dogumTarihi = new Date(convertDate(dogumYeri.split("\n")[1]));
        console.log(durumu.slice(-3));
        var olumTarihi = new Date(convertDate(durumu.slice(-1) !== "-" ? durumu.split("\n")[1] : ""));
        var yas = Math.ceil((olumTarihi.getTime() - dogumTarihi.getTime()) / (1000 * 3600 * 24 * 365));
        console.log(yas);
